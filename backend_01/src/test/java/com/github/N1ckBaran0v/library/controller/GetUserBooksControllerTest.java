...
class GetUserBooksControllerTest {
    @Mock
    ...
    @InjectMocks
    private GetUserBooksController controller;
    ...
    void searchSelfSuccess() {
        ...
        given(bookService.findUserBooks("username", user)).willReturn(arr);
    ...
    void searchAnotherSuccess() {
        ...
        given(userService.getUsername("sessionId")).willReturn("username");
        given(userService.getUser("sessionId")).willReturn(user);
        given(bookService.findUserBooks("anotherUsername", user)).willReturn(arr);
        controller.handle(ctx);
        verify(ctx).setResponseBody(arr);
    }
    ...
    void forbiddenFailure() {
        ...
        willThrow(ForbiddenException.class).given(bookService).findUserBooks(any(), any());
        controller.handle(ctx);
